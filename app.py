...
ALLOWED_CATEGORIES = {"地理", "歴史", "公民", "生物", "化学", "地学", "物理", "天体"}
ALLOWED_CAT_BY_SUBJECT = {
    "社会": {"地理", "歴史", "公民"},
    "理科": {"生物", "化学", "地学", "物理", "天体"},
}
SUBJECT_CATEGORY_MASTER = {
    "社会": ["地理", "歴史", "公民"],
    "理科": ["生物", "化学", "地学", "物理", "天体"],
}
SUBJECT_UNIT_MASTER = {
    ...
    "理科": {
        "物理": ["身近な物理現象", "電流とその利用", "運動とエネルギー", "科学技術と人間"],
        "化学": ["身の回りの物質", "化学変化と原子・分子", "化学変化とイオン"],
        "生物": ["いろいろな生物とその共通点", "生物の体のつくりと働き", "生命の連続性", "自然と人間"],
...
最優先は、ユーザーの質問にまっすぐ答えることです。
見出し・番号・ラベル（例: ①②③、1)2)、【結論】、結論：、要点：）は使わないでください。
会話文として自然につなげ、まずは短めに答え、必要なら補足してください。
理由・書き方のコツ・具体例・次にやることは、毎回必須ではなく必要なときだけ入れてください。
質問が曖昧なときだけ、確認質問を1つ入れてください（毎回は不要）。
...
def api_masters():
    return jsonify({
        "ok": True,
        "subjects": sorted(ALLOWED_SUBJECTS),
        "categories": SUBJECT_CATEGORY_MASTER,
        "units": SUBJECT_UNIT_MASTER,
    }), 200
